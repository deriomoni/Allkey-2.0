...
# ── T6–T7. Смешанный контракт — теперь выход за периметр ──────────────────
#
# Прежние редакции этих кейсов считали налог: T6 выделял шефмонтаж и давал
# 1 000 000 ₸, T7 без выделения облагал всю сумму. Решение владельца
# от 21.08.2026 по итогам разбора семнадцати консультаций: выплаты за товары
# выводятся из расчёта целиком. Развилок четыре, и каждая переворачивает
# ответ с «налога нет» на 20 %, а закрыть их анкетой нельзя — все требуют
# товаросопроводительных документов и фактических обстоятельств.
#
# Кейсы сохранены и переписаны на новое ожидание: сумм по этим веткам
# не возвращается ни в каком виде.

def test_T6_goods_with_split_services_is_out_of_scope():
    """Даже когда стоимость работ выделена, расчёта не будет.

    Выделение снимает одну развилку из четырёх — остальные три остаются,
    и каждая меняет ответ. Считать по одной закрытой развилке значило бы
    выдать уверенный ответ на неполных данных.
    """
...
def test_T7_goods_without_split_is_out_of_scope_too():
    """И без выделения — тот же выход, а не облагаемая совокупная сумма."""
    v = run(base(**{"S2.3": "CN", "S5.1": GOODS, "S5.2": "yes", "S5.3": "yes",
    ...
    assert v.out_of_scope == "goods"
    assert v.kpn.amount_kzt == 0
    assert "F-MIXED" not in v.flags        # предупреждать не о чем: расчёта нет
