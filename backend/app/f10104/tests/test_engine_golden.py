...
def test_golden_russia_dividends_share_70():
    """7 000 000 ₸ × 15 % = 1 050 000 ₸ · код 1100 · флаг F-DIV-25.

    Сознательная позиция продукта: при доле ≥ 25 % применяем 15 % по пп. 5),
    а не 5 % по пп. 6), и ставим предупреждение. См. ТЗ §7 и CLAUDE.md.
    """
    v = run(base(
        **{
            "S1.1": Q1_2026,
    ...
    assert v.graphs.get("G") in (None, "")     # R-FORM-04: графа G для дивидендов не заполняется
